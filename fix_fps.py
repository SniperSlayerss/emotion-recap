...
Re-mux session videos at the correct framerate.
...
figure out the true framerate, then re-muxes the h264 data (if available)
or the existing mp4 stream into a correctly-paced mp4.

No re-encoding — frames are copied, only the container framerate changes.
Fast, lossless.

Usage:
    python fix_video_fps.py <parent_dir>                # dry-run, shows plan
    python fix_video_fps.py <parent_dir> --apply        # actually fix

    python fix_video_fps.py <single_session> --apply    # one session
