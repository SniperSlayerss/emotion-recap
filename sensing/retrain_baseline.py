...
Retrain the Isolation Forest on BASELINE-ONLY sessions.

The original model was trained on mixed data (baseline + horror + etc.), which
means its "normal" distribution is too broad — so baseline sessions still flag
at 40–60%. This script retrains using only sessions whose label starts with
'baseline_', so "normal" = calm, and horror/stress sessions will score well
below threshold.
