...
def print_summary(df, gsr, hrv, meta):
    ...
    if meta:
        ...
        print(
            f"  Duration   : {meta.get('duration_s', '?'):.0f} s  ({meta.get('duration_s', 0) / 60:.1f} min)"
        )
...
def plot_session(session_dir: Path) -> None:
    ...
    Generates all 6 plots and saves them to <session_dir>/plots/.
    """
...
def plot_all_sessions(parent_dir: Path) -> None:
    """
...
def main():
    if len(sys.argv) < 2:
        sys.exit(
            "Usage: python plot_session.py <path>\n\n"
            "  <path> can be:\n"
            "    - A single session directory (contains features.csv)\n"
            "    - A parent directory whose sub-folders are sessions"
        )
    ...
    if (target / "features.csv").exists():
        ...
        # Batch mode — treat it as a parent of session dirs
        plot_all_sessions(target)
