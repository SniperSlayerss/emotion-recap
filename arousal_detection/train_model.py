...
Train a single Isolation Forest on *merged* GSR+HRV feature vectors
(one model, 8 features per row — each row combines a GSR window with a
nearby HRV window).

Kept for comparison with the ensemble approach in train_ensemble.py.
Prefer train_ensemble.py for new work — it trains two independent
sub-models and avoids the train/inference distribution mismatch caused
by half-imputing missing modalities.
