...
def score_session_df(
    ...
    skip = {
        ...
        "ensemble_mode",
        "gsr_anomaly_score", "gsr_anomaly_normalised", "gsr_is_aroused",
        "hrv_anomaly_score", "hrv_anomaly_normalised", "hrv_is_aroused",
    }

    # ------------------------------------------------------------------
    # Helper: extract a clean feature dict from a row
    # ------------------------------------------------------------------
    ...
    if isinstance(detector, EnsembleDetector):
        # Step 1: score each row independently against its matching
        # sub-model. Write raw + normalised into the output.
        for i, row in df.iterrows():
            ...
            # Provisional flag = this source's own flag; possibly overridden
            # by the combine step below for 'all' / 'mean'.
            out.at[i, "_is_aroused"] = bool(result.is_aroused)

        # Step 2: apply the combine mode via the shared helper, which
        # pairs GSR ↔ HRV rows on the timeline. It expects the long-form
        # columns (time_s, source, score, normalised, is_aroused, scored).
        ...
        if combine_mode in ("all", "mean") and scored_mask.any():
            ...
            mt = (mean_threshold if mean_threshold is not None
            ...
            # Map combined flags back via (time_s, source) which together
            # uniquely identify a scored row. We match on round-tripped
            # time to survive float noise.
            key_to_flag = {
                (round(float(r["time_s"]), 3), r["source"]): bool(r["is_aroused"])
                for _, r in combined.iterrows()
            }
            for i in out.index[scored_mask]:
                key = (round(float(out.at[i, "window_start_time"]), 3),
                       out.at[i, "source"])
    ...
    kind = "ensemble:" + combine_mode if isinstance(detector, EnsembleDetector) else "single"
    print(f"  Scored {n_scored}/{len(out)} windows — "
...
def plot_arousal_timeline(scored: pd.DataFrame, plots_dir: Path, meta: dict) -> None:
    ...
    # With the ensemble, each row's threshold is its own sub-model's — so
    # there may be two distinct values (GSR and HRV). We'll plot them as
    # separate horizontal lines per source.
...
def plot_session(
    ...
    if model_path is not None:
        ...
        try:
            scored = score_session_df(df, model_path,
...
def main():
    ...
    parser.add_argument("--combine-mode", choices=("any", "all", "mean"), default="any",
