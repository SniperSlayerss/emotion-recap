...
from arousal_detector import EnsembleDetector, load_detector
...
def merge_events(flags: list[dict], merge_gap: float) -> list[dict]:
    ...
    groups: list[list[dict]] = [[flags[0]]]
    for f in flags[1:]:
        if f["time_s"] - groups[-1][-1]["time_s"] <= merge_gap:
            groups[-1].append(f)
        else:
            groups.append([f])
    return [_event_summary(group) for group in groups]
...
def split_long_events(
    ...
    """Split any event whose resulting clip would exceed max_duration."""
    out = []
    for ev in events:
...
def main() -> int:
    ...
    # Score + combine
    raw = score_session(csv_path, detector, tolerance_s=args.tolerance)
    if is_ensemble:
        results = combined_timeline(
            raw,
    ...
    flags = [
        ...
        if bool(r.get("scored", True)) and bool(r["is_aroused"])
    ]
